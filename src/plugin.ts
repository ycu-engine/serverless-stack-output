...
export default class StackOutputPlugin {
  ...
  constructor(
    ...
    this.hooks = {
      'after:deploy:deploy': process,
    }

    if (this.serverless.service.custom.output.hooks) {
      this.serverless.service.custom.output.hooks.forEach((hook) => {
        this.hooks[hook] = process
      })
    }
    ...
    return this.hasHandler()
      ? this.callHandler(data).then(() =>
          this.serverless.cli.log(
            util.format(
              'Stack Output processed with handler: %s',
              this.output.handler
            )
          )
        )
